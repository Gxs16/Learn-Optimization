...
@Description: 在原始动态规划的基础上增加了多进程，小数据集上效果不理想
'''
...
def solve(adj_matrix, memo, start_node, num_nodes, pool):
    '''
    求解动态规划方程
    param\n
    adj_matrix: 邻接矩阵，二维列表
    ...
    for num_subset in range(3, num_nodes+1):
        print('{time}|子集大小：{num}'.format(time=datetime.now(), num=num_subset))
        time_list.append(datetime.now())
        # 对元素个数为3个以上的集合进行搜索
        for subset in combinations(num_subset, num_nodes):
            print('{time}|搜索子集：{num}'.format(time=datetime.now(), num=bin(subset)))
            result = []
            # 对每种可能的组合进行搜索
            ...
            for next_node in range(num_nodes):
                ...
                if (next_node == start_node) or not_in(next_node, subset):
                    ...
                    continue
                result.append(pool.apply_async(func=search, args=(subset, next_node, num_nodes, memo, start_node, adj_matrix)))
            for res in result:
                _next_node, min_dist = res.get()
                memo[_next_node][subset] = min_dist
    print(time_list)
            

def search(subset, next_node, num_nodes, memo, start_node, adj_matrix):
    # subset = args[0]
    # next_node = args[1]
    # num_nodes = args[2]
    # memo = args[3]
...
def tsp(data):
    ...
    solve(adj_matrix, memo, start_node, num_nodes, pool)
